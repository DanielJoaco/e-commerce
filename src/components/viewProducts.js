...
const ViewProduct = () => {
  const { name } = useParams(); // Captura el nombre del producto desde la URL
  ...
  // Buscar el producto al cargar el componente
  useEffect(() => {
    const matchedProduct = Object.values(data)
      .flat()
      .find(
        (item) =>
    ...
    <div className="view-product-container">
      {alert.show && (
        <Stack className="alert-container" spacing={2}>
          <Alert severity={alert.severity} className="alert-message">
            {alert.message}
          </Alert>
        </Stack>
      )}
